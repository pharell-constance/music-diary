...
function ProfileHero({ profileUser, isOwnProfile, connected, livePlaying, onEditClick, onFollowToggle, onReportUser, onOpenFollowModal, onLogoutClick }) {
    ...
    return (
        <div
            ref={heroRef}
            className={`p-4 md:p-8 pt-10 md:pt-12 bg-gradient-to-b ${connected ? 'from-violet-950/20' : 'from-white/[0.01]'} to-transparent border-b border-white/[0.05]`}
        >
            <div className="flex flex-col lg:flex-row justify-between items-center lg:items-start gap-8 w-full">
                {/* Left Part: Avatar + Info */}
                <div className="flex flex-col md:flex-row items-center md:items-start gap-6 text-center md:text-left flex-1 min-w-0 w-full">
                    ...
                    <div className="hero-meta flex-1 space-y-3 min-w-0 w-full">
                        <span className="inline-block text-[10px] font-black uppercase tracking-widest text-violet-400 bg-violet-500/10 px-3 py-1.5 rounded-full border border-violet-500/20">
                ...
                {/* Right Part: Fav Artist + Actions */}
                <div className="flex flex-col sm:flex-row lg:flex-col items-center lg:items-end gap-4 w-full md:w-auto mt-4 lg:mt-0 flex-shrink-0">
                    {/* Artiste Préféré */}
                    {profileUser.favArtistId && (
                        <div 
                            onClick={() => navigate(`/artist/${profileUser.favArtistId}`)}
                            className="neobrutal-card p-3.5 flex items-center gap-3.5 cursor-pointer transition-all duration-300 w-full sm:w-60 flex-shrink-0 rotate-[2deg] hover:rotate-0"
                        >
                    ...
                    {/* Actions rapides */}
                    <div className="hero-action flex flex-col sm:flex-row lg:flex-col items-center lg:items-end gap-3 w-full sm:w-auto">
                        {isOwnProfile ? (
                            <div className="flex flex-wrap gap-2 justify-center lg:justify-end w-full">
                                <button
                                    onClick={onEditClick}
                                    className="flex items-center gap-2 neobrutal-btn bg-[#1e1e22] hover:bg-zinc-800 text-white px-4 py-2.5 rounded-full font-mouse-memoirs uppercase tracking-widest text-sm"
                                >
                                    <Settings size={14} />
                                    Modifier le profil
                                </button>
                        ...
                        {connected && (
                            <div className="flex items-center gap-2 bg-violet-500/10 text-violet-400 px-4 py-2 rounded-full border border-violet-500/20 font-semibold text-xs self-center lg:self-auto">
